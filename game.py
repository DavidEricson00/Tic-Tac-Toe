...
def game(player1, player2):
    ...
    while turn <= 9 and not winner:  # Game loop, if table is full or someone wins, stop
        ...
        try:
            x, y = current_player.coordenate()  # Get coordinates

            if x < 0 or x > 2 or y < 0 or y > 2:  # Check if the coordinates are out of bounds
                print("Invalid value, please try again.")

            elif table[x][y] != ' ':  # Check if the position is already occupied
                print("Position already occupied, please try again.")

            else:
                table[x][y] = current_player.symbol  # Add symbol to table
                turn += 1  # Pass turn
